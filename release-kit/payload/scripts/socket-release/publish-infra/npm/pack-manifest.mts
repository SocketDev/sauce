 ...
 *   manifest that packs. The fleet manifest declares consumer-visible lifecycle
 *   scripts (`preinstall` → scripts/socket-release/setup/…) whose targets are
 *   repo scaffolding the `files` field never ships, so the published tarball's
 *   manifest points at files it does not carry and every consumer install
 *   breaks (the sdk 4.0.3 incident). Same shape as the README pin in
 ...
 *   release, stage, direct, approve-time verify re-pack, release-asset pack so
 *   the integrity gates keep comparing identical bytes. npm-only: cargo
