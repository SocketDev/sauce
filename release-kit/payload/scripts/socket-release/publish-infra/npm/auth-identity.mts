 ...
 *   npm; this module owns the orthogonal failure: logged in as the WRONG user.
 *   Staged entries are visible only to the subject package's maintainers, so a
 *   non-maintainer login makes `pnpm stage list` read as EMPTY and every
 *   verify/approve silently no-ops — the operator debugs "0 staged entries"
 *   instead of "wrong account". ensureNpmIdentity reads who is needed, the
 *   packument's maintainers, who is logged in (`npm whoami`), and on mismatch
 *   prompts for the logout/login rotation on a TTY or fails LOUD with the exact
 *   commands otherwise. The maintainer read is a three-way discriminant — known
 *   / unpublished / unreachable — because only a 404, first publish, may pass
 *   silently: a transient registry failure on a KNOWN-published package would
 *   otherwise fail open and re-open the exact wrong-account trap this gate
 *   closes. npm commands run from npmScratchCwd() — see its doc for why the
 *   temp dir is the only cwd that dodges both the repo's devEngines veto and
 *   lib spawn's untrusted-root PATH sanitization. Also a CLI: `node
 *   scripts/socket-release/publish-infra/npm/auth-identity.mts <package>`.
 */
...
async function runCli(): Promise<void> {
  ...
  if (!pkg) {
    logger.fail(
      'Usage: node scripts/socket-release/publish-infra/npm/auth-identity.mts <package>',
    )
    process.exitCode = 1
    return
  }
