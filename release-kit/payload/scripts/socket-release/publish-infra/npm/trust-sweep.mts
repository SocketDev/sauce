 ...
 *   The law per package matches the shape the browser plan derived: github ·
 *   file npm-publish.yml · repo <the consumer's slug> · environment
 *   npm-publish · permissions createPackage +
 ...
 *   Usage: node scripts/socket-release/publish-infra/npm/trust-sweep.mts
 *   <pkg>… --repo <owner/name> [--drive]
 */
...
/**
 * The one lawful trusted-publisher shape for a kit consumer: github ·
 * workflow npm-publish.yml · environment npm-publish · permissions
 * createPackage + createStagedPackage — only the repository slug varies.
 */
export interface TrustedPublisherLaw {
  environment: string
  file: string
...
export async function sweepOne(
  pkg: string,
  config: { drive: boolean; repository: string },
): Promise<SweepResult> {
  const cfg = { __proto__: null, ...config } as typeof config
  // The file/env/permission law is fleet-constant; only the repository varies
  // by where the package lives (a member package → its own repo via --repo).
  const law = trustedPublisherLaw(cfg.repository)
  try {
    const current = await trustList(pkg)
    if (current && conformsToLaw(current, law)) {
    ...
    if (!cfg.drive) {
      const from = current
        ...
        : '(no config)'
      return {
        detail: `[dry-run] ${from} -> ${law.file} @ ${law.repository} / env ${law.environment}`,
        pkg,
        status: 'planned',
      }
    }
    ...
    if (!echoed || !conformsToLaw(echoed, law)) {
      return {
...
async function main(): Promise<void> {
  ...
  if (packages.length === 0) {
    logger.fail('no packages: pass package names.')
    process.exitCode = 1
    return
  }
  if (!repoOverride) {
    logger.fail(
      'no repository: pass --repo <owner/name> — the law binds each package ' +
        'to the repo whose npm-publish.yml workflow publishes it.',
    )
    process.exitCode = 1
    return
  }
    ...
    try {
      ...
      if (e instanceof TrustAuthDiedError) {
        ...
        const reopened = await reopenAuthWindow(
          pkg,
          trustedPublisherLaw(repoOverride!),
        )
        if (!reopened) {
          logger.fail(e.message)
          logger.log(
            'No web-auth flow was offered — the token itself is dead, not ' +
              'just the 2FA window. Fix: node scripts/socket-release/npm-web-auth.mts ' +
