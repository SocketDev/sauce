 ...
 * the bytes the registry received + write a `checksums.txt` (sha1 + sha256 +
 * sha512), returning both paths. The sha256 line is the one a Homebrew formula
 * bump reads: brew-publish.mts derives every formula sha256 from the release's
 * own checksum manifest and never re-hashes an asset itself, so a release cut
 * without a sha256 line is a release no formula can be bumped against. Returns
 * an empty array, with a warning, when the pack fails, so the release still
 * lands without assets. Extracted so `ensureTagAndRelease` can accept an
 * alternate packer without changing the npm behavior.
 */
async function defaultPackAssets(pkg: {
  ...
  writeThroughMirrorLock(
    checksumsPath,
    formatReleaseChecksums(tarballName, bytes),
  )
 ...
 */
export function formatReleaseChecksums(
  assetName: string,
  bytes: Buffer,
): string {
  const sha1 = crypto.createHash('sha1').update(bytes).digest('hex')
 ...
 * 3-step draft → upload → undraft flow. Assets are the tarball packed from this
 * same tree in this same run — the identical bytes the registry just received —
 * plus a checksums file (sha1 + sha256 + sha512), so the GitHub-release shasum
 * is directly comparable to the npm staged/published shasum.
 *
