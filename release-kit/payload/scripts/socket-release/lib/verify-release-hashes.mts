...
function buildMismatchMessage(
  ...
  return (
    ...
    `  Fix: reject the staged publish (node scripts/socket-release/npm-web-auth.mts stage reject <stageId>) and re-run the release — never approve a divergent artifact.`
  )
}
