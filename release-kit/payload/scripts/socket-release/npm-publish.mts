 ...
 *   (`publish-infra/npm/` — registry reads, staged/direct modes, and the
 *   approve flow).
 */
...
async function main(): Promise<void> {
  ...
  if (values['help']) {
    ...
    logger.log(
      '                       content. Bypasses the version-order gate behind',
    )
    logger.log(
  ...
  const checkoutRef =
    typeof values['checkout-ref'] === 'string' && values['checkout-ref']
      ? values['checkout-ref']
      : undefined
  // The kit defers CI auto-bump (`--bump` / `--release-as`), so those two
  // conflict arms are structurally unreachable here — the flags do not exist.
  // The gate still runs: `--checkout-ref` without `--backfill`, and
  // `--backfill` outside the staged path, are the live arms.
  ...
  // The version-order gate is bypassed — the backfill guards replace it —
  ...
  if (mode === 'staged') {
    ...
    // operator's bump commit is on origin. Rebase our remaining local commits
