/**
 * @file Pure helpers + types backing `install-claude-plugins.mts`'s reconciler.
 *   Split out to keep the CLI entry under the 500-line soft cap — these are
 *   also the surface `scripts/repo/test/install-claude-plugins.test.mts`
 ...
 * (e.g. `<x>.files/scripts/lib/read-stdin-sync.mjs` →
 * `<cache>/scripts/repo/lib/…`). The fleet "smallest patch footprint" rule
 * prefers moving substantial logic into such a sidecar module so the diff
 * itself stays an import + call-site swap, rather than inlining a 30-line
 * function body. Returns the dir path (whether or not it exists — caller
 * checks).
 */
