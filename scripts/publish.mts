 ...
 *   consumed directly from this repository. Usage: node scripts/publish.mts
 *   Regenerate all artifacts. node scripts/publish.mts --check Regenerate, then
 *   fail if tracked output changed.
 */
...
const GENERATORS = [
  'scripts/inline-shared.mts',
  'scripts/sync-versions.mts',
  'scripts/generate-agents.mts',
  'scripts/generate-cursor-plugin.mts',
] as const
...
async function check(): Promise<void> {
  ...
  await runScript('scripts/generate-cursor-plugin.mts', '--check')
  await runScript('scripts/inline-shared.mts', '--check')
...
export async function main(args = process.argv.slice(2)): Promise<void> {
  ...
  if (arg === undefined) {
    ...
    process.stdout.write(
      'Usage: node scripts/publish.mts [--check]\n' +
