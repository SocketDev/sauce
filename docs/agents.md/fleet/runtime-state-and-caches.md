...
- **Never write runtime state into the tracked tree.** Local/per-checkout state
  ...
  - **dep-0 (can't import socket-lib) → `node_modules/.cache/<name>/`.** The
    ...
    exist, so it is strictly dep-0 — `node_modules/.cache/fleet-scaffolding/` is
...
- **`bundle.ref`** (`.config/fleet.json`) — a CUSTOM pin (not a
  ...
  runtime state.
- **`node_modules/.cache/fleet-scaffolding/bundle-applied`** (dep-0 fetcher
  ...
  `cat node_modules/.cache/fleet-scaffolding/bundle-applied`; clear: delete it (or
