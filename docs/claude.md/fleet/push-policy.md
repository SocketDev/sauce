...
When either is set to the literal string `"true"`, the cascade's `canSkipReviewGate()` check (in `the-fleet-repo/scripts/_shared/repo-properties.mts`) allows direct push for routine cascade work. Anything else (`"false"`, `"Choose the value"` placeholder, missing entirely, API failure) falls back to "open a PR".
