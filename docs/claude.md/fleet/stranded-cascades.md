...
The wheelhouse cascade runs `the-fleet-repo/scripts/fleet/cleanup-stranded.mts --target <repo>` against each fleet repo **before** creating that wave's `chore/wheelhouse-<sha>` worktree. Default mode is **fix**:
...
The wheelhouse cascade enforces this by running `cleanup-stranded.mts` against the target repo **before** creating wave N+1's worktree. Same call site as the supersession cleanup below, just with the "vs origin" check now extended to "vs the next-wave SHA we're about to use."
...
The script lives at `the-fleet-repo/scripts/fleet/cleanup-stranded.mts`. You don't normally run it directly (the cascade does that), but it's safe to invoke ad-hoc:

```bash
# Dry-run against one repo (substitute the actual repo path).
node $PROJECTS/the-fleet-repo/scripts/fleet/cleanup-stranded.mts \
...
# Apply the fix.
node $PROJECTS/the-fleet-repo/scripts/fleet/cleanup-stranded.mts --all
```
...
4. **Template SHA that's not a strict ancestor**: the local commit may be from a branch of `the-fleet-repo/template/` that was never merged. Confirm by inspecting the SHA in the wheelhouse history (`git -C $PROJECTS/the-fleet-repo log <sha>`). If it's orphan / abandoned, `git reset --hard origin/<base>` manually after backing up the SHA in case it's wanted later.
