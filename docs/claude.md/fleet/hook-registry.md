...
- **`.claude/hooks/fleet/<name>/`** — fleet-canonical hooks. Edited only in `the-fleet-repo/template/.claude/hooks/fleet/<name>/`; cascade pushes to every fleet repo. Citation gate (`new-hook-claude-md-guard`) requires each hook to have a matching `(enforced by ...)` mention somewhere in CLAUDE.md or the linked fleet docs.
