...
- A path: use the **absolute path** (`/Users/<user>/projects/the-fleet-repo/tools/...`; write personal segments as the `<user>` placeholder per `personal-path-placeholders`), not a bare basename or "that file" — absolute is unambiguous across worktrees and parallel sessions.
