...
Fleet-canonical files (anything tracked by `the-fleet-repo/scripts/sync-scaffolding/manifest.mts`) MUST be edited in `the-fleet-repo/template/...` and cascaded out. Never branched locally in a downstream fleet repo.
...
If unsure, check `the-fleet-repo/scripts/sync-scaffolding/manifest.mts`. Tracked = canonical.

## How to apply

If a downstream repo needs a behavior change in one of these files:

1. Edit the file in `the-fleet-repo/template/...`.
...
- **DO NOT** grep a downstream repo's copy of `.config/fleet/oxlint-plugin/`, `.claude/hooks/fleet/`, `.git-hooks/`, `docs/claude.md/fleet/`, or the `CLAUDE.md` fleet block to check what's in it. Read from `the-fleet-repo/template/...` instead.
...
If a cascaded file genuinely seems wrong, the fix lives in `the-fleet-repo/template/...`, never in the downstream copy. Open the template file in `the-fleet-repo/`, read it there, edit it there, cascade.
...
- The **canonical block** between `BEGIN/END FLEET-CANONICAL` markers IS fleet-canonical. Apply the no-fork rule + the trust-the-wheelhouse rule there. Edit only in `the-fleet-repo/template/CLAUDE.md` and cascade.
